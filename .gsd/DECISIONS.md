...
| D011 | M002/S02 | architecture | Agreements page data source | Real on-chain data via program.account.paymentAgreement.all() with 30s refetch polling, replacing hardcoded mock data | Fund button requires real Created payments to operate on; mock data cannot support actual on-chain transactions. Separate AgreementsClient component keeps the page as a server shell while client component handles wallet-dependent data fetching. | Yes |
