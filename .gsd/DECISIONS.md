...
| D012 | M003/S02 | arch | Permissioned action component pattern | Extract each permissioned action (MultisigAction, WebhookAction) as its own sub-component with dedicated useWallet + useMutation + TransactionStatus state, separate from permissionless CrankAction | Permissioned actions need wallet identity gating (PublicKey.equals checks) and their own mutation lifecycle; sharing TransactionStatus with CrankAction causes state collision | Yes — could consolidate if a generic PermissionedAction abstraction emerges |
